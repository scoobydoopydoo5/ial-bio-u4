  ...
  const loadPosts = async () => {
    ...
    if (postsData) {
      const formattedPosts = postsData.map((post: any) => {
        const likes =
          post.threads_post_likes?.filter((l: any) => l.is_like).length || 0;
        const dislikes =
          post.threads_post_likes?.filter((l: any) => !l.is_like).length || 0;
        const userLikeObj = post.threads_post_likes?.find(
  ...
  const loadComments = async (postId: string) => {
    ...
    if (data) {
      const formattedComments = data.map((comment: any) => {
        const likes =
          comment.threads_comment_likes?.filter((l: any) => l.is_like).length ||
          0;
        const dislikes =
          comment.threads_comment_likes?.filter((l: any) => !l.is_like)
            .length || 0;
        const userLikeObj = comment.threads_comment_likes?.find(
          (l: any) => l.user_id === user?.id
        );
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-4">
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      requireAuth(() => togglePostLike(post.id, true))
                    }
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      requireAuth(() => togglePostLike(post.id, false))
                    }
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            className={
                              comment.userLike === true ? "text-green-500" : ""
                            }
                          >
                          ...
                          <Button
                            ...
                            className={
                              comment.userLike === false ? "text-red-500" : ""
                            }
                          >
