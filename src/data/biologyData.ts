import { Topic } from "@/types/checklist";

export const initialBiologyData: Topic[] = [
  {
    id: "topic-5",
    title: "T5 Energy Flow, Ecosystems and the Environment",
    description:
      "Photosynthesis, energy flow, ecosystems, succession, and climate change.",
    collapsed: false,
    hidden: false,
    lessons: [
      {
        id: "lesson-5-1",
        title: "Photosynthesis and Energy Capture",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-5-1-1",
            text: "Understand the overall reaction of photosynthesis as requiring energy from light to split apart the strong bonds in water molecules, storing the hydrogen in a fuel (glucose) by combining it with carbon dioxide and releasing oxygen into the atmosphere.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-1-2",
            text: "Understand how photophosphorylation of ADP requires energy and that hydrolysis of ATP provides an immediate supply of energy for biological processes.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-1-3",
            text: "Understand the light-dependent reactions of photosynthesis, including how light energy is trapped by exciting electrons in chlorophyll and the role of these electrons in generating ATP, reducing NADP in cyclic and non-cyclic photophosphorylation, and producing oxygen through photolysis of water.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-1-4",
            text: "(i) Understand the light-independent reactions as reduction of carbon dioxide using the products of the light-dependent reactions (carbon fixation in the Calvin cycle, the role of GP, GALP, RuBP, and RUBISCO).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-1-4-2",
            text: "(ii) Know that the products are simple sugars that are used by plants, animals, and other organisms in respiration and the synthesis of new biological molecules (polysaccharides, amino acids, proteins, lipids, and nucleic acids).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-1-5",
            text: "Understand the structure of chloroplasts in relation to their role in photosynthesis.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-1-6",
            text: "Understand what is meant by the terms absorption spectrum and action spectrum.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-1-7",
            text: "Understand that chloroplast pigments can be separated using chromatography and the pigments identified using Rf values.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-1-8",
            text: "Core Practical 1 (10): Investigate the effects of light intensity, light wavelength, temperature, and availability of carbon dioxide on the rate of photosynthesis using a suitable aquatic plant.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-5-2",
        title: "Productivity and Energy Transfers",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-5-2-1",
            text: "(i) Understand the relationship between gross primary productivity (GPP), net primary productivity (NPP), and plant respiration (R).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-2-1",
            text: "(ii) Be able to calculate net primary productivity.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-2-2",
            text: "Know how to calculate the efficiency of biomass and energy transfers between trophic levels.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-5-3",
        title: "Ecology: Populations, Communities and Succession",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-5-3-1",
            text: "Understand what is meant by the terms population, community, habitat, and ecosystem.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-3-2",
            text: "Understand that the numbers and distribution of organisms in a habitat are controlled by biotic and abiotic factors.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-3-3",
            text: "Understand how the concept of niche accounts for the distribution and abundance of organisms in a habitat.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-3-4",
            text: "Core Practical 2 (11): Carry out a study of the ecology of a habitat, such as using quadrats and transects to determine the distribution and abundance of organisms, and measuring abiotic factors appropriate to the habitat.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-3-5",
            text: "Understand the stages of succession from colonization to the formation of a climax community.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-5-4",
        title: "Climate Change and Environmental Impact",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-5-4-1",
            text: "Understand the different types of evidence for climate change and its causes, including records of carbon dioxide levels, temperature records, pollen in peat bogs, and dendrochronology, recognizing correlations and causal relationships.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-4-2",
            text: "Understand the causes of anthropogenic climate change, including the role of greenhouse gases in the greenhouse effect.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-4-3",
            text: "Understand how knowledge of the carbon cycle can be applied to methods to reduce atmospheric levels of carbon dioxide.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-4-4",
            text: "(i) Understand that data can be extrapolated to make predictions and that these are used in models of future climate change.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-4-4-2",
            text: "(ii) Understand that models for climate change have limitations.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-4-5",
            text: "Understand the effects of climate change (changing rainfall patterns and changes in seasonal cycles) on plants and animals (distribution of species, development, and life cycles).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-4-6",
            text: "Understand the effect of temperature on the rate of enzyme activity and its impact on plants, animals, and microorganisms, to include Q10.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-4-7",
            text: "Core Practical 3 (12): Investigate the effects of temperature on the development of organisms (such as seedling growth rate or brine shrimp hatch rates), taking into account the ethical use of organisms.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-5-5",
        title: "Evolution and Conservation",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-5-5-1",
            text: "Understand how evolution (a change in allele frequency) can come about through gene mutation and natural selection.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-5-2",
            text: "Understand how isolation reduces gene flow between populations, leading to allopatric or sympatric speciation.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-5-3",
            text: "Understand the way in which scientific conclusions about controversial issues, such as what actions should be taken to reduce climate change, or the degree to which humans are affecting climate change, can sometimes depend on who is reaching the conclusions.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-5-5-4",
            text: "Understand how reforestation and the use of sustainable resources, including biofuels, are examples of the effective management of the conflict between human needs and conservation.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
        ],
      },
    ],
  },
  {
    id: "topic-6",
    title: "T6 Microbiology, Immunity and Forensics",
    description: "Microorganisms, immune response, and forensics.",
    collapsed: false,
    hidden: false,
    lessons: [
      {
        id: "lesson-6-1",
        title: "Microbial Growth and Techniques",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-6-1-1",
            text: "Understand the principles and techniques involved in culturing microorganisms, using aseptic technique.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-1-2",
            text: "Understand the different methods of measuring the growth of microorganisms, as illustrated by cell counts, dilution plating, mass, and optical methods (turbidity).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-1-3",
            text: "Understand the different phases of a bacterial growth curve (lag phase, exponential phase, stationary phase, and death phase) and be able to calculate exponential growth rate constants.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-1-4",
            text: "Core Practical 4 (13): Investigate the rate of growth of microorganisms in a liquid culture, taking into account the safe and ethical use of organisms.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-6-2",
        title: "Infection and Immune Response",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-6-2-1",
            text: "(i) Be able to compare the structure of bacteria and viruses (nucleic acid, capsid structure, and envelope) with reference to Ebola virus, tobacco mosaic virus (TMV), human immunodeficiency virus (HIV), and lambda phage (λ phage).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-2-1-2",
            text: "(ii) Understand what is meant by the terms lytic and latency.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-2-2",
            text: "Understand how Mycobacterium tuberculosis and human immunodeficiency virus (HIV) infect human cells, causing symptoms that may result in death.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-2-3",
            text: "(i) Know the major routes pathogens may take when entering the body.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-2-3",
            text: "(ii) Understand the role of barriers in protecting the body from infection, including skin, stomach acid, and gut and skin flora.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-6-3",
        title: "Immune System and Pathogens",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-6-3-1",
            text: "Understand the non-specific responses of the body to infection, including inflammation, lysozyme action, interferon, and phagocytosis.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-3-2",
            text: "Understand the roles of antigens and antibodies in the body’s immune response including the involvement of plasma cells, macrophages, and antigen-presenting cells.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-3-3",
            text: "Understand the differences between the roles of B cells (B memory and B effector cells), and T cells (T helper, T killer, and T memory cells) in the host’s immune response.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-3-4",
            text: "Understand how individuals may develop immunity (natural, artificial, active, and passive).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-6-4",
        title: "Antibiotics, Microbial Resistance, and Forensics",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-6-4-1",
            text: "Understand how the theory of an ‘evolutionary race’ between pathogens and their hosts is supported by evasion mechanisms shown by pathogens.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-4-2",
            text: "Understand the difference between bacteriostatic and bactericidal antibiotics.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-6-4-3",
            text: "Core Practical 5 (14): Investigate the effect of different antibiotics on bacteria.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
        ],
      },
    ],
  },
  {
    id: "topic-7",
    title: "T7 Respiration, Muscles and the Internal Environment",
    description:
      "Respiration, muscle structure and function, cardiac function, homeostasis, and kidney function.",
    collapsed: false,
    hidden: false,
    lessons: [
      {
        id: "lesson-7-1",
        title: "Aerobic and Anaerobic Respiration",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-7-1-1",
            text: "(i) Understand the overall reaction of aerobic respiration as splitting of the respiratory substrate to release CO2 and reuniting hydrogen with O2 with release of energy.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-1-2",
            text: "(ii) Understand that respiration is a stepped process, with each step catalysed by a specific enzyme.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-1-3",
            text: "Understand the roles of glycolysis in aerobic and anaerobic respiration, including phosphorylation of hexoses, production of ATP by substrate-level phosphorylation, reduced coenzymes, pyruvate, and lactate.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-1-4",
            text: "Understand the link reaction and Krebs cycle in complete oxidation of glucose: CO2 production, ATP by substrate-level phosphorylation, reduced NAD/FAD by dehydrogenation, occurring in mitochondria.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-1-5",
            text: "Understand how ATP is synthesised by oxidative phosphorylation via electron transport chain, chemiosmosis, and ATP synthase.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-1-6",
            text: "Understand what happens to lactate after anaerobic respiration in animals.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-1-7",
            text: "Understand the term respiratory quotient (RQ).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-1-8",
            text: "Core Practical 15: Use an artificial hydrogen carrier (redox indicator) to investigate respiration in yeast.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-1-9",
            text: "Core Practical 16: Use a simple respirometer to determine the rate of respiration and RQ of a suitable material.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-7-2",
        title: "Muscle Structure and Function",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-7-2-1",
            text: "Understand how muscles, tendons, skeleton and ligaments interact to enable movement, including antagonistic pairs, extensors and flexors.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-2",
            text: "(i) Know the structure of a mammalian skeletal muscle fibre.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-3",
            text: "(ii) Understand structural and physiological differences between fast and slow twitch fibres.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-4",
            text: "Understand skeletal muscle contraction (sliding filament theory): actin, myosin, troponin, tropomyosin, Ca2+, ATP, ATPase.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-5",
            text: "(i) Know the myogenic nature of cardiac muscle.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-6",
            text: "(ii) Understand coordination of heartbeat: SAN, AVN, bundle of His, Purkyne fibres.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-7",
            text: "(iii) Understand how ECGs aid in diagnosis of abnormal heart rhythms.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-8",
            text: "(i) Calculate cardiac output.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-9",
            text: "(ii) Understand variations in ventilation and cardiac output for oxygen delivery and CO2 removal, including heart rate and ventilation control by medulla.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-10",
            text: "Understand the role of adrenaline in fight or flight response.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-2-11",
            text: "Core Practical 17: Investigate effects of exercise on tidal volume, breathing rate, respiratory minute ventilation, and oxygen consumption.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-7-3",
        title: "Homeostasis and Kidney Function",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-7-3-1",
            text: "(i) Understand negative and positive feedback control.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-3-2",
            text: "(ii) Understand the principle of negative feedback in maintaining systems within narrow limits.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-3-3",
            text: "Understand homeostasis and its importance in maintaining dynamic equilibrium during exercise, including hypothalamus in thermoregulation.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-3-4",
            text: "Know gross and microscopic structure of mammalian kidney.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-3-5",
            text: "Understand how urea is produced in the liver from excess amino acids and removed via ultrafiltration.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-3-6",
            text: "Understand selective reabsorption in the proximal tubule and how the loop of Henle acts as countercurrent multiplier.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-3-7",
            text: "Understand ADH, pituitary gland, and osmoreceptors in plasma concentration and blood volume control.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-7-3-8",
            text: "Understand how genes can be switched on/off by transcription factors and hormones.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
        ],
      },
    ],
  },
  {
    id: "topic-8",
    title: "T8 Coordination, Response and Gene Technology",
    description:
      "Nervous system, reflexes, plant responses, brain structure, and gene technology.",
    collapsed: false,
    hidden: false,
    lessons: [
      {
        id: "lesson-8-1",
        title: "Nervous System and Reflexes",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-8-1-1",
            text: "Know structure and function of sensory, relay, and motor neurones, including Schwann cells and myelination.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-2",
            text: "Understand how the nervous system causes effectors to respond to a stimulus.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-3",
            text: "Know structure and function of spinal reflex arc, including grey and white matter of spinal cord.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-4",
            text: "Understand nerve impulse conduction along an axon, including sodium/potassium permeability changes.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-5",
            text: "Understand role of myelination in saltatory conduction.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-6",
            text: "(i) Know structure and function of synapses, neurotransmitters, acetylcholine.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-7",
            text: "(ii) Understand how the pupil dilates and contracts.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-8",
            text: "Understand drug effects on nerve impulses (nicotine, lidocaine, cobra venom α-toxin, L-DOPA, MDMA).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-9",
            text: "Understand how rods detect stimuli, roles of rhodopsin, opsin, retinal, sodium ions, hyperpolarisation in optic neurones.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-10",
            text: "Understand habituation. Recommended practical: investigate habituation to a stimulus.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-1-11",
            text: "Know central and peripheral nervous system.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
        ],
      },
      {
        id: "lesson-8-2",
        title: "Plant Responses and Gene Technology",
        collapsed: false,
        hidden: false,
        objectives: [
          {
            id: "obj-8-2-1",
            text: "Understand how phytochrome, auxin (IAA), and gibberellins bring about responses in plants, including transcription effects.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-2",
            text: "Core Practical 18: Investigate production of amylase in germinating cereal grains.",
            completed: false,
            tags: [
              { id: "tag-practical", label: "Core Practical", color: "green" },
            ],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-3",
            text: "Understand coordination in animals via nervous and hormonal control.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-4",
            text: "Know location and functions of cerebral hemispheres, hypothalamus, pituitary, cerebellum, medulla oblongata.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-5",
            text: "Understand MRI, fMRI, PET, and CT for brain investigation.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-6",
            text: "Understand imbalances in brain chemicals (dopamine, serotonin) and drug development.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-7",
            text: "Know how drugs are produced using GMOs (plants, animals, microorganisms).",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-8",
            text: "Understand recombinant DNA production: restriction endonucleases, DNA ligase.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-9",
            text: "Understand insertion of recombinant DNA into other cells.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-10",
            text: "Know microarrays can identify active genes.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-11",
            text: "Understand bioinformatics.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
          {
            id: "obj-8-2-12",
            text: "Understand risks and benefits of GMOs.",
            completed: false,
            tags: [],
            comments: [],
            hidden: false,
          },
        ],
      },
    ],
  },
];
